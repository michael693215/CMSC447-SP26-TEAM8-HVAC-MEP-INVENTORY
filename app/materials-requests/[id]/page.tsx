...
// Kept your updated STATUS_STYLES with "completed"
...
export default async function RequestDetailsPage({ 
  ...
  // Your logic to determine if it's pending to toggle column visibility
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 text-black">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Teammate's Header card styling */}
        ...
        <div className="bg-white shadow-md rounded-lg overflow-x-auto border border-gray-200">
          <table className="w-full text-left border-collapse">
            <thead className="table-header-accent">
              <tr>
                {/* Teammate's Sticky Column Header */}
                <th className="p-3 sm:p-4 border-b sticky left-0 z-20 bg-blue-200">#</th>
                <th className="p-3 sm:p-4 border-b">Material</th>
                
                {/* Your Dynamic Headers using teammate's padding styles */}
                {isPending ? (
                  <>
                    <th className="p-3 sm:p-4 border-b text-center">Expecting</th>
                    <th className="p-3 sm:p-4 border-b text-center">Delivered</th>
                  </>
                ) : (
                  <th className="p-3 sm:p-4 border-b text-center">Delivered Qty</th>
                )}
            ...
            <tbody>
              {request.items.length === 0 ? (
                <tr>
                  {/* Updated colSpan dynamically based on your logic */}
                  <td colSpan={isPending ? 6 : 5} className="p-10 text-center text-gray-500 italic">No items on this request.</td>
                </tr>
              ) : (
                request.items.map((item: any) => {
                  
                  // Your expectation math logic
                  ...
                  return (
                    <tr key={item.line_number} className="hover:bg-blue-50 transition-colors border-b border-gray-100">
                      ...
                      {/* Teammate's Sticky Column styling */}
                      ...
                      {/* Your Dynamic Cells merged with teammate's large font styling */}
                      {isPending ? (
                        <>
                          <td className="p-3 sm:p-4 text-center font-mono text-lg font-bold text-blue-600">{expecting}</td>
                          <td className="p-3 sm:p-4 text-center font-mono text-lg font-bold text-green-600">{delivered}</td>
                        </>
                      ) : (
                        <td className="p-3 sm:p-4 text-center font-mono text-lg font-bold text-green-600">{delivered}</td>
                      )}
